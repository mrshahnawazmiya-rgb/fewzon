      ...
      <Container className="relative z-10 w-full px-4 sm:px-6 md:px-8">
        <div className="max-w-5xl mx-auto text-center">
          ...
          delay: 0.1
            ...
            Build something <span className="text-gradient glow">​Fewzon</span>
          </motion.h1>
          ...
          delay: 0.3
            ...
            <div className="bg-black/60 backdrop-blur-sm rounded-lg border border-white/10 overflow-hidden shadow-lg">
              <div className="p-4">
                <textarea value={prompt} onChange={e => setPrompt(e.target.value)} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} placeholder="What would you like to build today?" rows={3} className="w-full bg-transparent border-0 p-2 text-base text-white leading-relaxed placeholder:text-white/50 focus:ring-0 resize-none scrollbar-none" />
                ...
                <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/10">
                  ...
                  <Button onClick={handleGenerate} disabled={isGenerating} className="bg-gradient-to-r from-bluepill to-redpill hover:opacity-90">
                    ...
                    repeat: Infinity,
                    ease: "linear"
                  }} className="w-5 h-5 border-2 border-t-white border-white/30 rounded-full" /> : "Generate"}
                  </Button>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {pills.map(pill => <motion.button key={pill.id} onClick={() => setPrompt(`Create a ${pill.label.toLowerCase()}`)} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            duration: 0.4,
            delay: 0.4 + pills.indexOf(pill) * 0.1
          }} whileHover={{
            scale: 1.05
          }} className="flex items-center px-3 py-1.5 bg-white/10 hover:bg-white/15 border border-white/10 rounded-full text-xs text-white/80 transition-all">
